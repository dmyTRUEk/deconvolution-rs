...
impl PatternSearchScaledStep {
    pub fn fit(&self, deconvolution_data: &DeconvolutionData) -> FitResult {
        ...
        let PatternSearchScaledStep { fit_algorithm_min_step, fit_residue_evals_max, fit_residue_max_value, initial_step, alpha, beta } = self.clone();
        ...
        let initial_params: Params = deconvolution_data.get_initial_params();
...
impl Load for PatternSearchScaledStep {
    ...
    fn load_from_self_toml_value(toml_value: &TomlValue) -> Self {
        let fit_algorithm_min_step = toml_value
            ...
            .expect("fit_params -> pattern_search_scaled_step: `fit_algorithm_min_step` not found")
            .as_float()
            .expect("fit_params -> pattern_search_scaled_step -> fit_algorithm_min_step: can't parse as float");
        let fit_residue_evals_max = toml_value
            .get("fit_residue_evals_max")
            .expect("fit_params -> pattern_search_scaled_step: `fit_residue_evals_max` not found")
            .as_integer()
            .expect("fit_params -> pattern_search_scaled_step -> fit_residue_evals_max: can't parse as integer");
        let fit_residue_max_value = toml_value
            .get("fit_residue_max_value")
            .expect("fit_params -> pattern_search_scaled_step: `fit_residue_max_value` not found")
            .as_float()
            .expect("fit_params -> pattern_search_scaled_step -> fit_residue_max_value: can't parse as float");
        assert!(fit_residue_evals_max > 0);
        let fit_residue_evals_max = fit_residue_evals_max as u64;
        let initial_step = toml_value
            .get("initial_step")
            .expect("fit_method_params -> pattern_search_scaled_step: `initial_step` not found")
            .as_float()
            .expect("fit_method_params -> pattern_search_scaled_step -> initial_step: can't parse as float");
        let alpha = toml_value
            .get("alpha")
            .expect("fit_method_params -> pattern_search_scaled_step: `alpha` not found")
            .as_float()
            .expect("fit_method_params -> pattern_search_scaled_step -> alpha: can't parse as float");
        let beta = if let Some(beta_toml_value) = toml_value.get("beta") {
            Some(
                beta_toml_value
                    .as_float()
                    .expect("fit_method_params -> pattern_search_scaled_step -> beta: can't parse as float")
            )
        } else {
            None
        };
